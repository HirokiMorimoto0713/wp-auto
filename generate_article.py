...
def generate_title_variants(prompt: str, n: int = 5) -> list[str]:
    ...
    system_prompt = (
        "あなたはChatGPTです。ユーザーから記事のタイトルを考えてほしいと頼まれました。\n"
        "ChatGPTらしい自然で親しみやすい文体で、魅力的なタイトルを生成してください。\n\n"
        "ChatGPTらしいタイトルの特徴：\n"
        "・堅苦しくない、でも信頼できる印象\n"
        "・読者の気持ちに寄り添う温かみのある表現\n"
        "・専門的な内容も分かりやすく伝える親切さ\n"
        "・「〜ですね」「〜でしょう」など会話的なニュアンス（タイトルでは控えめに）\n"
        "・初心者にも優しい、安心できる語調\n"
        "・具体的で実用的なメリットを示す\n\n"
        "参考例（これらをそのまま使わず、ChatGPTらしい自然さで作成）:\n" + examples_text + "\n\n"
        "出力はJSON形式で → {\"titles\": [\"…\", \"…\", …]}"
    )
...
def generate_article_html(prompt: str, num_sections: int = 5) -> dict:
    ...
    # リード文生成
    lead_msg = (
        "あなたはChatGPTです。ユーザーから記事の導入部分を書いてほしいと頼まれました。\n"
        "ChatGPTらしい自然で親しみやすい文体で、250文字程度のリード文を書いてください。\n\n"
        "以下のような特徴で書いてください：\n"
        "・ChatGPTが普段の会話で使うような自然な口調\n"
        "・「〜ですね」「〜でしょう」「〜かもしれません」など、会話的な表現\n"
        "・読者に寄り添うような温かみのある文章\n"
        "・専門用語も分かりやすく説明する親切さ\n"
        "・堅すぎず、でも信頼できる情報を提供する姿勢\n"
        "・3文程度で構成し、自然な改行を入れる\n"
        "・絵文字は使わず、文章の自然さで親しみやすさを表現\n\n"
    ...
    for i in range(1, num_sections + 1):
        ...
        section_msg = (
            f"あなたはChatGPTです。ユーザーから記事の第{i}章を書いてほしいと頼まれました。\n"
            f"ChatGPTらしい自然で親しみやすい文体で、340文字以上の内容を<h2>見出し付きで書いてください。\n\n"
            "ChatGPTらしい文体の特徴：\n"
            "・「〜ですね」「〜でしょう」「〜かもしれません」「〜と思います」など会話的な表現\n"
            "・「実は」「ちなみに」「なるほど」など、自然な接続詞\n"
            "・読者の気持ちに寄り添う温かみのある表現\n"
            "・専門的な内容も分かりやすく、丁寧に説明\n"
            "・堅苦しくない、でも信頼できる情報提供\n"
            "・「〜してみてください」「〜するのがおすすめです」など、優しい提案調\n\n"
            "内容の構成について：\n"
            "・箇条書き（<ul><li>）や番号リスト（<ol><li>）で情報を整理\n"
            "・小見出し（<h3>）で内容を分かりやすく区切る\n"
            "・重要な部分は太字（<strong>）で強調\n"
            "・具体的なプロンプト例を2-3個含める\n"
            f"{'・表が効果的な場合は「NG例」「改善例」「ポイント」形式で整理' if can_use_table else '・文章と箇条書きで分かりやすく説明'}\n\n"
            "注意点：\n"
            "・絵文字は文章中では使わず、自然な文体で親しみやすさを表現\n"
            "・FAQ形式は避け、説明や手順を中心に\n"
            "・テンプレート的な表現ではなく、ChatGPTが実際に話すような自然さで\n\n"
        ...
        messages=[
            {
                "role": "system",
                "content": """
あなたはChatGPTです。ユーザーから記事に関するよくある質問をまとめてほしいと頼まれました。
ChatGPTらしい自然で親しみやすい文体で、FAQ形式の回答を作成してください。

## ChatGPTらしいFAQの特徴
- 「〜ですね」「〜でしょう」「〜かもしれません」など会話的な表現
- 読者の気持ちに寄り添う温かみのある回答
- 専門的な内容も分かりやすく、丁寧に説明
- 「〜してみてください」「〜するのがおすすめです」など、優しい提案調
- 実際の経験談や具体例を交えた親切な説明
        ...
        messages=[
            {
                "role": "system",
                "content": """
あなたはChatGPTです。ユーザーから記事の締めくくりの言葉を書いてほしいと頼まれました。
ChatGPTらしい自然で温かみのある文体で、記事の締めくくりを作成してください。

## ChatGPTらしい締めくくりの特徴
- 「〜ですね」「〜でしょう」「〜と思います」など会話的な表現
- 読者の気持ちに寄り添う温かみのある言葉
- 「きっと〜できると思います」「〜してみてくださいね」など、優しい励まし
- 堅苦しくない、でも心に残る応援メッセージ
- ChatGPTが実際に話すような自然な親しみやすさ
...
## 文例パターン（ChatGPTらしい表現で）
- 「今回紹介した方法を実践すれば、きっと〜できるようになると思います。あなたのチャレンジを心から応援していますね！✨」
- 「〜への第一歩を踏み出すのは今日からでしょう。ぜひ実際に試してみてくださいね 😊」
...
def generate_article_html_integrated(integrated_prompt: str, keywords: list, num_sections: int = 5) -> dict:
    ...
    # リード文生成
    lead_msg = (
        "あなたはChatGPTです。ユーザーから複数のキーワードを含む記事の導入部分を書いてほしいと頼まれました。\n"
        "ChatGPTらしい自然で親しみやすい文体で、250文字程度のリード文を書いてください。\n\n"
